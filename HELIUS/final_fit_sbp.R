...
saveRDS(RS_fit_pt1,'output/RS_fit_pt1_sbp.Rda')    
saveRDS(rules,'output/rules_sbp.Rda')
saveRDS(base_ind,'output/base_ind_sbp.Rda')
...
RS_fit_pt1=readRDS('output/RS_fit_pt1_sbp.Rda')
...
saveRDS(RS_fit_pt2,'output/RS_fit_pt2_sbp.Rda')
...
library(dplyr)
library(ggplot2)
ruleshap_df=readRDS('output/final_shapleys_sbp.Rda')[[2]]
...
#Explore interaction between ethnicity and age
ruleshap_df=readRDS('output/final_shapleys_sbp.Rda')[[1]]
sub_df=ruleshap_df[ruleshap_df$predictor %in% c('age','ethnicity'),]
...
ggplot(data=sub_df,
  ...
  geom_point(aes(shape=factor(x.2)),alpha=.6)+
  geom_line(data=lines_df, linewidth=1)+
  theme(legend.position = "top", 
        legend.box = "horizontal",
        legend.direction = "horizontal")+
  guides(color = guide_legend(nrow = 1))+
  xlab('Age')+ylab('Joint Age + Ethnicity effect')+
  scale_color_manual(values=c(colorspace::rainbow_hcl(5)),
                     name='Ethnicity')+
...
shapleys_df=readRDS('output/final_shapleys_sbp.Rda')[[1]]
